define(['./directive'], function(directive) {
  ...
  directive.directive('titleOne', [
    '$session', '$dateFormat',
    function($session, $dateFormat) {
      return {
        restrict: 'E',
        replace: true,
        templateUrl: 'html/template/title1.html',
        controller: [
          '$scope', '$element',
          function($scope, $element) {
            $scope.checkType = $element.attr('check-type') || 'multiple';
            ...
            var juegeSearchCan = function() {
              var smallDate = new Date(replaceWords($scope.startDate)),
                bigDate = new Date(replaceWords($scope.endDate)),
                date;
